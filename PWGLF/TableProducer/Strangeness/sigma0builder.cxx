...
struct sigma0builder {
  ...
  // Configurable<bool> fRejectPi0{"fRejectPi0", false, "Flag to reject pi0."}; // this is experimental!
  ...
  // {
  //   if (((gamma.mGamma() >= 0) && (gamma.mGamma() < 0.02)) && ((lambda.mGamma() >= 0) && (lambda.mGamma() < 0.02))){
  ...
  void processMonteCarlo(soa::Join<aod::StraCollisions, aod::StraCents> const& collisions, V0DerivedMCDatas const& V0s)
  {
    for (const auto& coll : collisions) {
      ...
      for (auto& gamma : V0Table_thisCollision) {    // selecting photons from Sigma0
        //bool skipGamma = false; // Flag to skip the current gamma (only works using fRejectPi0)!
        ...
        for (auto& lambda : V0Table_thisCollision) { // selecting lambdas from Sigma0
          ...
          // if (fRejectPi0) {
          //   if (processPi0Rejection(lambda, gamma)) // basic selection
          //     skipGamma = true;
        ...
        //if (skipGamma) continue; // Skip to the next gamma candidate (only works using fRejectPi0)!
      }
    }
  }
