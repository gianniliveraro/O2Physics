...
struct sigma0builder {
  ...
  void init(InitContext const&)
  {
    ...
    if (doprocessV0QA || doprocessV0MCQA) {
      ...
      histos.add("V0QA/h3dTruePhotonMassCheckedDaughters", "h3dTruePhotonMassCheckedDaughters", kTH3D, {axisCentrality, axisPt, axisPhotonMass});
      histos.add("V0QA/h2dTrueSigma0PhotonMassCheckedDaughters", "h2dTrueSigma0PhotonMassCheckedDaughters", kTH2D, {axisPt, axisPhotonMass});
      ...
      histos.add("V0QA/h3dTrueLambdaMassCheckedDaughters", "h3dTrueLambdaMassCheckedDaughters", kTH3D, {axisCentrality, axisPt, axisLambdaMass});
      histos.add("V0QA/h2dTrueSigma0LambdaMassCheckedDaughters", "h2dTrueSigma0LambdaMassCheckedDaughters", kTH2D, {axisPt, axisLambdaMass});
      ...
      histos.add("V0QA/h3dTrueALambdaMassCheckedDaughters", "h3dTrueALambdaMassCheckedDaughters", kTH3D, {axisCentrality, axisPt, axisLambdaMass});
      histos.add("V0QA/h2dTrueSigma0ALambdaMassCheckedDaughters", "h2dTrueSigma0ALambdaMassCheckedDaughters", kTH2D, {axisPt, axisLambdaMass});
      ...
      for (size_t i = 0; i < v0grouped[coll.globalIndex()].size(); i++) {
        ...
        if constexpr (requires { v0.motherMCPartId(); }) {          
          ...
          if (v0.has_v0MCCore()) {
            ...
            int V0PDGCodeNeg = v0MC.pdgCodeNegative();
            int V0PDGCodePos = v0MC.pdgCodePositive();

            bool isPositiveProton = V0PDGCodePos == PDG_t::kProton;
            bool isPositivePion = V0PDGCodePos == PDG_t::kPiPlus || (lambdaSelections.doTreatPiToMuon && V0PDGCodePos == PDG_t::kMuonPlus);
            bool isNegativeProton = V0PDGCodeNeg == PDG_t::kProtonBar;
            bool isNegativePion = V0PDGCodeNeg == PDG_t::kPiMinus || (lambdaSelections.doTreatPiToMuon && V0PDGCodeNeg == PDG_t::kMuonMinus);

            // True Photon
            if (v0MC.pdgCode() == PDG_t::kGamma && IsPhysicalPrimary && fPassPhotonSel) {
              
              // All V0s tagged as true photons
              ...
              // Only those with conversion daughters 
              if (V0PDGCodeNeg==PDG_t::kElectron && V0PDGCodePos==PDG_t::kPositron){
                histos.fill(HIST("V0QA/h3dTruePhotonMassCheckedDaughters"), centrality, v0MC.ptMC(), v0.mGamma());
                if (TMath::Abs(v0MC.pdgCodeMother()) == PDG_t::kSigma0) { // If from sigma0 or ASigma0
                  histos.fill(HIST("V0QA/h2dTrueSigma0PhotonMassCheckedDaughters"), v0MC.ptMC(), v0.mGamma());
                }
              }
              
              if (V0PDGCodeNeg==PDG_t::kPositron && V0PDGCodePos==PDG_t::kElectron){
                LOGF(info, "WARNING! Positive=electron and Negative=positron! Weird!");
              }
            ...
            // True Lambda
            if (v0MC.pdgCode() == PDG_t::kLambda0 && IsPhysicalPrimary && fPassLambdaSel) {
              ...
              if (isPositiveProton && isNegativePion){
                histos.fill(HIST("V0QA/h3dTrueLambdaMassCheckedDaughters"), centrality, v0MC.ptMC(), v0.mLambda());
                if (TMath::Abs(v0MC.pdgCodeMother()) == PDG_t::kSigma0) { // If from sigma0 or ASigma0
                  histos.fill(HIST("V0QA/h2dTrueSigma0LambdaMassCheckedDaughters"), v0MC.ptMC(), v0.mLambda());
                }
              }
            ...
            // True ALambda
            if (v0MC.pdgCode() == PDG_t::kLambda0Bar && IsPhysicalPrimary && fPassLambdaSel) {
              ...
              if (isPositivePion && isNegativeProton){
                histos.fill(HIST("V0QA/h3dTrueALambdaMassCheckedDaughters"), centrality, v0MC.ptMC(), v0.mAntiLambda());
                if (TMath::Abs(v0MC.pdgCodeMother()) == PDG_t::kSigma0Bar) { // If from sigma0 or ASigma0
                  histos.fill(HIST("V0QA/h2dTrueSigma0ALambdaMassCheckedDaughters"), v0MC.ptMC(), v0.mAntiLambda());
                }
              }
            }

            // True KShort
            if (v0MC.pdgCode() == PDG_t::kK0Short && IsPhysicalPrimary && fPassKShortSel) {
