...
using V0DerivedMCDatas = soa::Join<aod::V0Cores, aod::V0CollRefs, aod::V0MCDatas>;
using V0MLDerivedDatas = soa::Join<aod::V0Cores, aod::V0CollRefs, aod::V0LambdaMLScores, aod::V0GammaMLScores, aod::V0AntiLambdaMLScores>;
using V0StandardDerivedDatas = soa::Join<aod::V0Cores, aod::V0CollRefs>;
...
struct sigma0builder {
  ...
  void fillTables(TV0Object const& lambda, TV0Object const& gamma)
  {
    ...
    float fPhotonPosTPCNSigma = 3;//posTrackGamma.tpcNSigmaEl();
    float fPhotonNegTPCNSigma = 3;//negTrackGamma.tpcNSigmaEl();
    uint8_t fPhotonPosTPCCrossedRows = 160;//posTrackGamma.tpcCrossedRows();
    uint8_t fPhotonNegTPCCrossedRows = 160;//negTrackGamma.tpcCrossedRows();
    ...
    int fPhotonPosITSCls = 6;//posTrackGamma.itsNCls();
    int fPhotonNegITSCls = 6;//negTrackGamma.itsNCls();
    uint32_t fPhotonPosITSClSize = 6;//posTrackGamma.itsClusterSizes();
    uint32_t fPhotonNegITSClSize = 6;//negTrackGamma.itsClusterSizes();
    ...
    float fLambdaPosPrTPCNSigma = 3;//posTrackLambda.tpcNSigmaPr();
    float fLambdaPosPiTPCNSigma = 3;//posTrackLambda.tpcNSigmaPi();
    float fLambdaNegPrTPCNSigma = 3;//negTrackLambda.tpcNSigmaPr();
    float fLambdaNegPiTPCNSigma = 3;//negTrackLambda.tpcNSigmaPi();
    uint8_t fLambdaPosTPCCrossedRows = 160;//posTrackLambda.tpcCrossedRows();
    uint8_t fLambdaNegTPCCrossedRows = 160;//negTrackLambda.tpcCrossedRows();
    ...
    int fLambdaPosITSCls = 6;//posTrackLambda.itsNCls();
    int fLambdaNegITSCls = 6;//negTrackLambda.itsNCls();
    uint32_t fLambdaPosITSClSize = 6;//posTrackLambda.itsClusterSizes();
    uint32_t fLambdaNegITSClSize = 6;//negTrackLambda.itsClusterSizes();
  ...
  void processMonteCarlo(soa::Join<aod::StraCollisions, aod::StraCents> const& collisions, V0DerivedMCDatas const& V0s)
  {
    for (const auto& coll : collisions) {
      ...
      for (auto& gamma : V0Table_thisCollision) {    // selecting photons from Sigma0
        ...
        if (gamma.pdgCode() == 3122)
          histos.fill(HIST("Efficiency/h2dPtVsCentrality_LambdaAll"), centrality, gamma.pt()); // islambda
  ...
  void processSTDSelection(soa::Join<aod::StraCollisions, aod::StraCents> const& collisions, V0StandardDerivedDatas const& V0s)
  {
    for (const auto& coll : collisions) {
      // Do analysis with collision-grouped V0s, retain full collision information
      const uint64_t collIdx = coll.globalIndex();
  ...
  void processMLSelection(soa::Join<aod::StraCollisions, aod::StraCents> const& collisions, V0MLDerivedDatas const& V0s)
  {
    for (const auto& coll : collisions) {
      // Do analysis with collision-grouped V0s, retain full collision information
      const uint64_t collIdx = coll.globalIndex();
