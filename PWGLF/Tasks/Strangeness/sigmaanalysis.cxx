...
struct sigmaanalysis{
  ...
  void processMonteCarlo(aod::StraCollision const& coll, soa::Join<aod::V0Cores, aod::V0CollRefs, aod::V0Extras, aod::V0MCDatas> const& v0s)
  {
    ...
    for (auto& gamma: v0s) { // selecting photons from Sigma0
      ...
      if ((gamma.pdgCode()!=22 || gamma.pdgCodeMother()!=3212) && (gamma.pdgCode()!=-22 || gamma.pdgCodeMother()!=-3212)) continue;

      for (auto& lambda: v0s){ // selecting lambdas from Sigma0
        if ((lambda.pdgCode()!=3122 || lambda.pdgCodeMother()!=3212) && (lambda.pdgCode()!=-3122 || lambda.pdgCodeMother()!=-3212)) continue;
        if (gamma.motherMCPartId()!=lambda.motherMCPartId()) continue; // selecting pair from exactly the same mother
